...
ninfer::glm53::TensorCatalog catalog_from(const std::vector<ninfer::glm53::ExpectedTensor>& expected) {
    ...
    for (const auto& tensor : expected) {
        ...
        if (tensor.mcg_payload) {
            observed.has_mcg_value = true;
            observed.mcg_value = ninfer::glm53::kExl3McgMultiplier;
        }
...
int main() {
    ...
    expect_eq(target.size(), 150226U, "target tensors");
    ...
    expect_eq(packed, 148608U, "packed tensors");
    ...
    const auto* trellis = find_logical(target, "language.layer.3.moe.expert.0.gate.trellis");
    ...
    expect(trellis != nullptr && trellis->dtype == DType::kI16 &&
               trellis->shape == std::vector<std::int64_t>({256, 128, 64}) &&
               trellis->source_name == "model.language_model.layers.3.mlp.experts.0.gate_proj.trellis",
           "expert trellis");
    ...
    const auto* nextn_mla = find_logical(target, "language.nextn.mla.kv_b_proj");
    ...
    expect(nextn_mla != nullptr && nextn_mla->shape == std::vector<std::int64_t>({32768, 512}), "nextn kv_b");
    ...
    constexpr std::string_view kTargetHash = "f3a76ffad53259bbde06e7bbf43a138d0e5af60344cb5a1d8305058572328a90";
    ...
    const auto round_trip = bind_catalog(CheckpointKind::kGlm53Exl3Target, target, catalog_from(target), true);
    ...
    expect(loaded.tensors.at("tiny.weight").dtype == "BF16" &&
           ...
           "tiny weight");
    expect(loaded.tensors.at("tiny.mcg").has_mcg_value && loaded.tensors.at("tiny.mcg").mcg_value == kExl3McgMultiplier,
           "tiny mcg");

    ExpectedTensor mcg;
    mcg.logical_id = "probe.mcg";
    mcg.source_name = "tiny.mcg";
    mcg.dtype = DType::kI32;
    mcg.storage = StorageClass::kExl3McgI32;
    mcg.shape = {1};
    mcg.mcg_payload = true;
    TensorCatalog mcg_only;
    mcg_only.tensors.emplace("tiny.mcg", loaded.tensors.at("tiny.mcg"));
    const auto mcg_ok = bind_catalog(CheckpointKind::kGlm53Exl3Target, {mcg}, mcg_only, true);
    expect(mcg_ok.complete, "mcg constant accepted");
    mcg_only.tensors.at("tiny.mcg").mcg_value = 1U;
    const auto mcg_bad = bind_catalog(CheckpointKind::kGlm53Exl3Target, {mcg}, mcg_only, true);
    expect(!mcg_bad.complete && mcg_bad.mcg_mismatches == 1, "mcg constant rejected");

    const auto name_dir = temp / "names";
    std::filesystem::create_directories(name_dir);
    write_text(name_dir / "config.json", R"({
  "architectures": ["Glm5NextForConditionalGeneration"],
  "quantization_config": {"bits": 4, "codebook": "mcg", "quant_method": "exl3", "scope": "glm53_routed_experts_only", "serving_reader_qualified": false},
  ...
  "vision_config": {"depth": 24}
    ...
    expect(names.kind == CheckpointKind::kGlm53Exl3Target, "names kind");
    ...
    expect(!names.warnings.empty(), "unqualified reader warning");
