...
void check_config(BindingReport& report, CheckpointKind kind, std::string_view json) {
    if (kind == CheckpointKind::kGlm53Exl3Target) {
        ...
        require_string(report, json, "quant_method", "exl3");
        require_string(report, json, "codebook", "mcg");
        require_string(report, json, "scope", "glm53_routed_experts_only");
        require_int(report, json, "bits", 4);
        ...
        const auto qualified = json_scan::find_bool(json, "serving_reader_qualified");
        if (!qualified || !*qualified) {
            report.warnings.emplace_back(
                "checkpoint marks serving_reader_qualified=false; binding checks storage, not a qualified runtime reader");
        }
        return;
    }
...
CheckpointKind detect_kind(std::string_view config, BindingReport& report) {
    ...
    if (target == draft) {
        ...
        return CheckpointKind::kGlm53Exl3Target;
    }
    return target ? CheckpointKind::kGlm53Exl3Target : CheckpointKind::kDflash2Draft;
}
...
std::string_view to_string(CheckpointKind kind) noexcept {
    switch (kind) {
        case CheckpointKind::kGlm53Exl3Target:
            return "glm53_flash_exl3_target";
...
BindingReport bind_catalog(CheckpointKind kind, const std::vector<ExpectedTensor>& expected,
    ...
    for (const auto& [name, observed_tensor] : observed.tensors) {
        ...
        if (required.mcg_payload) {
            if (!observed_tensor.has_mcg_value || observed_tensor.mcg_value != kExl3McgMultiplier) {
                ++report.mcg_mismatches;
                add_error(report, "mcg payload mismatch for " + required.logical_id);
            }
        }
    }
...
BindingReport bind_checkpoint_directory(const std::filesystem::path& directory, bool names_only) {
    ...
    const auto expected = report.kind == CheckpointKind::kGlm53Exl3Target ? expected_glm53_flash_tensors()
