...
// Routed-expert EXL3 linear. Sibling BF16 linears in this checkpoint are stored
// as [N, K] = [out, in]. The four storage tensors keep that orientation:
// suh[K], svh[N], trellis[K/16, N/16, 64], mcg[1].
void add_exl3(std::vector<ExpectedTensor>& out,
              const std::string& logical_prefix,
              const std::string& source_prefix,
              std::int64_t out_features,
              std::int64_t in_features) {
    if (out_features <= 0 || in_features <= 0 || out_features % 16 != 0 || in_features % 16 != 0) {
        throw std::logic_error("EXL3 linear features must be positive multiples of 16");
    }
    add(out, logical_prefix + ".suh", source_prefix + ".suh", DType::kF16, StorageClass::kExl3ScaleF16,
        {in_features}, false);
    add(out, logical_prefix + ".svh", source_prefix + ".svh", DType::kF16, StorageClass::kExl3ScaleF16,
        {out_features}, false);
    add(out, logical_prefix + ".trellis", source_prefix + ".trellis", DType::kI16, StorageClass::kExl3TrellisI16,
        {in_features / 16, out_features / 16, 64}, false);
    add(out, logical_prefix + ".mcg", source_prefix + ".mcg", DType::kI32, StorageClass::kExl3McgI32, {1}, true);
}
...
void add_moe(std::vector<ExpectedTensor>& out, const ModelSpec& model, const std::string& logical,
    ...
    for (std::uint32_t expert = 0; expert < model.routed_experts; ++expert) {
        const std::string expert_source =
            source + "mlp.experts." + std::to_string(expert) + ".";
        const std::string expert_logical = logical + "moe.expert." + std::to_string(expert) + ".";
        add_exl3(out, expert_logical + "gate", expert_source + "gate_proj", moe, hidden);
        add_exl3(out, expert_logical + "up", expert_source + "up_proj", moe, hidden);
        add_exl3(out, expert_logical + "down", expert_source + "down_proj", hidden, moe);
    }
}
...
void add_language_layer(std::vector<ExpectedTensor>& out, const ModelSpec& model, std::uint32_t index,
    ...
    if (ffn == FfnKind::kDense) {
        ...
        add_moe(out, model, logical, source);
    }
...
std::vector<std::string> validate_parameter_catalog(const std::vector<ExpectedTensor>& catalog) {
    ...
    for (const auto& tensor : catalog) {
        ...
        const bool mcg = tensor.storage == StorageClass::kExl3McgI32;
        if (tensor.mcg_payload != mcg) {
            errors.push_back("mcg payload flag does not match storage for " + tensor.logical_id);
        }
        ...
        if (tensor.storage == StorageClass::kExl3ScaleF16 && tensor.dtype != DType::kF16) {
            errors.push_back("EXL3 scale storage has the wrong dtype for " + tensor.logical_id);
        }
        if (tensor.storage == StorageClass::kExl3TrellisI16) {
            if (tensor.dtype != DType::kI16 || tensor.shape.size() != 3U || tensor.shape[2] != 64) {
                errors.push_back("EXL3 trellis storage shape is not [K/16, N/16, 64] for " + tensor.logical_id);
            }
        }
        if (mcg && (tensor.dtype != DType::kI32 || tensor.shape.size() != 1U || tensor.shape[0] != 1)) {
            errors.push_back("EXL3 mcg storage must be I32[1] for " + tensor.logical_id);
        }
    }
    return errors;
}
...
std::string_view to_string(StorageClass storage) noexcept {
    switch (storage) {
        ...
        case StorageClass::kNativeF32:
            return "native_f32";
        case StorageClass::kExl3ScaleF16:
            return "exl3_scale_f16";
        case StorageClass::kExl3TrellisI16:
            return "exl3_trellis_i16";
        case StorageClass::kExl3McgI32:
            return "exl3_mcg_i32";
    }
    return "unknown";
}
