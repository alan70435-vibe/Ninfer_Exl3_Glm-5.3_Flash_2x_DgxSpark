...
ValidationReport validate_deployment_contract(const DeploymentContract& d, const ModelSpec& model) {
    ...
    require_equal(report, "QUANTIZATION", d.quantization, "exl3");
    require_equal(report, "KV_CACHE_DTYPE", d.kv_cache_dtype, "fp8");
    if (d.model.find("GLM-5.3-Flash") == std::string::npos || d.model.find("EXL3") == std::string::npos) {
        report.errors.emplace_back("MODEL must identify a GLM-5.3-Flash EXL3 checkpoint");
    }
