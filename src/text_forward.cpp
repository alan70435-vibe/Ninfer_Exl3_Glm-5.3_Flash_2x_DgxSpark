...
float fp16_to_f32(std::uint16_t bits) {
    ...
    if (exp == 0u) {
        if (mant == 0u) {
            ...
            std::uint32_t magnitude = mant;
            int exponent = -14;
            while ((magnitude & 0x400u) == 0u) {
                magnitude <<= 1;
                --exponent;
            }
            magnitude &= 0x3ffu;
            out = sign | (static_cast<std::uint32_t>(exponent + 127) << 23) | (magnitude << 13);
        }
    } else if (exp == 31u) {
...
// Sylvester FWHT scaled by 1/sqrt(128), so the block is symmetric and an involution.
void fwht128(float* values) {
    for (int length = 1; length < 128; length <<= 1) {
        for (int start = 0; start < 128; start += length * 2) {
            for (int lane = 0; lane < length; ++lane) {
                const float left = values[start + lane];
                const float right = values[start + lane + length];
                values[start + lane] = left + right;
                values[start + lane + length] = left - right;
            }
        }
    }
...
public:
                ...
                if (name == "__metadata__") {
                    ...
                    while (true) {
                        ...
                        if (key == "dtype") {
                            ...
                            else if (dtype == "F16") tensor.kind = 'h';
                            else if (dtype == "I16") tensor.kind = 's';
                            else if (dtype == "I32") tensor.kind = 'i';
    ...
    void load(const std::string& name, float* dest, int n) const {
        ...
        if (tensor.kind != 'b' && tensor.kind != 'h') fail("cannot load " + name + " as float");
        const auto* bits = reinterpret_cast<const std::uint16_t*>(tensor.data);
        for (int i = 0; i < n; ++i) dest[i] = tensor.kind == 'b' ? bf16_to_f32(bits[i]) : fp16_to_f32(bits[i]);
    }
...
void accumulate_exl3_range(const std::uint16_t* trellis, const float* x_had, int in_features, int out_features,
                           int row0, int row1, float* y) {
    const int k_tiles = in_features / kExl3Tile;
    const int n_tiles = out_features / kExl3Tile;
    const int n0 = row0 / kExl3Tile;
    const int n1 = row1 / kExl3Tile;
    std::uint16_t perm[256];
    exl3_tensor_core_perm(perm);
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
    for (int n_tile = n0; n_tile < n1; ++n_tile) {
        for (int k_tile = 0; k_tile < k_tiles; ++k_tile) {
            const auto* packed = trellis + (static_cast<std::size_t>(k_tile) * static_cast<std::size_t>(n_tiles) +
                                             static_cast<std::size_t>(n_tile)) *
                                                static_cast<std::size_t>(kExl3PackedU16);
            ...
            for (int kernel_pos = 0; kernel_pos < 256; ++kernel_pos) {
                ...
                const int k = k_tile * kExl3Tile + row_major / kExl3Tile;
                const int n = n_tile * kExl3Tile + row_major % kExl3Tile;
                y[n] += mcg_symbol(symbols[kernel_pos]) * x_had[k];
            }
        }
    }
}
...
private:
    ...
    // Trellis stores Q in the Hadamard domain. su and sv stay outside it:
    // y = sv * H128(Q * H128(x * su)). Q comes from the MCG tiles only.
    void linear_exl3(const std::string& prefix, const float* x, int cols, float* y, int rows) {
        if (cols % 128 != 0 || rows % 128 != 0) fail("EXL3 Hadamard width " + prefix);
        const auto& trellis = store_.get(prefix + ".trellis");
        const auto& mcg = store_.get(prefix + ".mcg");
        if (trellis.kind != 's' || trellis.rank != 3 || dim_at(trellis, 0) != cols / 16 ||
            dim_at(trellis, 1) != rows / 16 || dim_at(trellis, 2) != 64) {
            fail("EXL3 trellis layout " + prefix);
        }
        std::uint32_t mcg_value = 0;
        std::memcpy(&mcg_value, mcg.data, sizeof(mcg_value));
        if (mcg.kind != 'i' || mcg_value != kExl3McgMultiplier) fail("EXL3 mcg constant " + prefix);
        std::vector<float> su(static_cast<std::size_t>(cols));
        std::vector<float> sv(static_cast<std::size_t>(rows));
        std::vector<float> x_had(static_cast<std::size_t>(cols));
        store_.load(prefix + ".suh", su.data(), cols);
        store_.load(prefix + ".svh", sv.data(), rows);
        for (int col = 0; col < cols; ++col) {
            x_had[static_cast<std::size_t>(col)] = x[col] * su[static_cast<std::size_t>(col)];
        }
        for (int block = 0; block < cols; block += 128) fwht128(x_had.data() + block);
        const auto* packed = reinterpret_cast<const std::uint16_t*>(trellis.data);
        apply_rows(0, rows, [&](int row0, int row1) {
            if (row0 % 128 != 0 || row1 % 128 != 0) fail("EXL3 row split " + prefix);
            std::fill(y + row0, y + row1, 0.f);
            accumulate_exl3_range(packed, x_had.data(), cols, rows, row0, row1, y);
            for (int block = row0; block < row1; block += 128) fwht128(y + block);
            for (int row = row0; row < row1; ++row) y[row] *= sv[static_cast<std::size_t>(row)];
        });
        if (link_fd_ >= 0) gather_rows(link_fd_, rank_, y, rows);
    }
    ...
    void dense_mlp(const std::string& prefix, const float* x, float* y) {
        ...
        linear_bf16(prefix + "mlp.gate_proj.weight", x, hidden_, buf_b_.data(), intermediate);
        linear_bf16(prefix + "mlp.up_proj.weight", x, hidden_, buf_c_.data(), intermediate);
        swiglu_clamp(buf_b_.data(), buf_c_.data(), intermediate, swiglu_limit_, buf_d_.data());
        linear_bf16(prefix + "mlp.down_proj.weight", buf_d_.data(), intermediate, y, hidden_);
    }
    ...
    void moe(const std::string& prefix, const float* x, float* y) {
        ...
        for (int pick = 0; pick < top_k_; ++pick) {
            ...
            linear_exl3(expert + "gate_proj", x, hidden_, buf_b_.data(), intermediate);
            linear_exl3(expert + "up_proj", x, hidden_, buf_c_.data(), intermediate);
            swiglu_clamp(buf_b_.data(), buf_c_.data(), intermediate, swiglu_limit_, buf_d_.data());
            linear_exl3(expert + "down_proj", buf_d_.data(), intermediate, buf_c_.data(), hidden_);
    ...
    void kda(int layer, const float* x, float* y) {
        ...
        if ((q_weight.kind != 'b' && q_weight.kind != 'f') || q_weight.rank < 3 || dim_at(q_weight, 0) != qkv_ ||
            dim_at(q_weight, 2) != kernel_) {
            fail("q conv layout");
        }
        std::vector<float> q_conv(static_cast<std::size_t>(qkv_ * kernel_));
        std::vector<float> k_conv(static_cast<std::size_t>(qkv_ * kernel_));
        std::vector<float> v_conv(static_cast<std::size_t>(qkv_ * kernel_));
        store_.load(prefix + "q_conv1d.weight", q_conv.data(), qkv_ * kernel_);
        store_.load(prefix + "k_conv1d.weight", k_conv.data(), qkv_ * kernel_);
        store_.load(prefix + "v_conv1d.weight", v_conv.data(), qkv_ * kernel_);
        causal_conv_silu(buf_b_.data(), q_conv.data(), cache.q_mem.data(), qkv_, kernel_, buf_b_.data());
        causal_conv_silu(buf_c_.data(), k_conv.data(), cache.k_mem.data(), qkv_, kernel_, buf_c_.data());
        causal_conv_silu(buf_d_.data(), v_conv.data(), cache.v_mem.data(), qkv_, kernel_, buf_d_.data());
...
void block_hadamard_128(float* data, int length) {
    if (length % 128 != 0) fail("Hadamard length must be a multiple of 128");
    for (int offset = 0; offset < length; offset += 128) fwht128(data + offset);
}
