...
Constructing layer 3 on that worker selected `FLASHINFER_MLA_SPARSE_SM120` and the `fp8_ds_mla` cache layout. The image logged `Sparse MLA impl has no dense-MHA prefill path; using the top-k MQA path only.` The unpacked module is about 3758 MiB, of which the routed trellis tensors are 2304 MiB and 1152 MiB. Its forward was not run. The recorded server continuation remains `13041,13041`, and the eager greedy remains `154822,154822`.
