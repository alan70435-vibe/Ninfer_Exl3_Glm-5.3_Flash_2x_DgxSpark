...
On 2026-09-22T23:45+08:00 `ninfer-glm53-generate` ran on `gx10-23ec` (NVIDIA GB10 12.1, `GPU-d5d58447-ec10-0155-e20a-cf12158797b9`), not on the head desktop. Prompt `13041`, greedy, `new_tokens=2`, revision `25a44fdbf16862a46b7cc9921142c6c81350af2f`, `world_size=1`. CUDA-graph capture fills input ids with token 0, and the image's block zeroer clears attention pages while skipping mamba. The generate path now steps token 0, drops the MLA cache, and keeps that KDA state. Each mHC stream is then stored as bf16, which is the serving hidden dtype. Leaving the streams in fp32 argmaxed at `22654` (logit `14.8386`, runner-up `13041` at `13.2771`). With the bf16 store, both repeats returned `token_ids=13041,13041`, `committed_length=2`, status OK. The first token logit was `12.3961` with runner-up `22654` at `12.313`. The second was `13041` at `17.213` with runner-up `0` at `14.5844`. The same worker's official layer walk, one step of token 0 then token `13041` with the MLA page cleared, also argmaxed at `13041` (logit `12.5625`) and the following token at `13041` (logit `17.375`). Host ctest was 11/11. Host MemAvailable was 122830860 KiB before the shipped runs and 122875244 KiB after. The second Spark was not used.

The same binary then ran `--mode dflash` twice on that worker, draft `dc77ff1c99eeb2df044ee3d4f0094eb033fee410`, seven proposals, `new_tokens=2`. Both repeats returned `token_ids=13041,13041`. Both local ranks committed length 2. `dflash_accept=bonus_only`, accept count 0, draft ids `11,264,13041,0,358,8355,13`. The committed ids are the target argmaxes; the draft tokens were rejected. `world_size=2` here is the process pair on one machine. `second_spark=not_used`. Host MemAvailable was 122884956 KiB before and 122893864 KiB after.

On 2026-09-23T00:10+08:00 the same entry ran as TP=2 with rank 0 bound to `192.168.100.10` on `gx10-5749` (`GPU-ca529c58-c4c3-2793-3566-260e0f97879a`) and rank 1 bound to `192.168.100.11` on `gx10-23ec` (`GPU-d5d58447-ec10-0155-e20a-cf12158797b9`). The ranks exchanged GEMV row halves over TCP on those CX-7 addresses, ports 29651 and 29652. Prompt `13041`, `new_tokens=2`, revision `25a44fdbf16862a46b7cc9921142c6c81350af2f`. Both repeats, on both ranks, returned `token_ids=13041,13041` and committed length 2. `second_spark=192.168.100.11`. Head MemAvailable stayed near 119437772 KiB and the worker near 122843596 KiB. Host ctest after the peer-link change was 11/11.

On 2026-09-22T22:50+08:00 the worker compared the shipped eager forward with the official 1-token decode, one stream-mean per layer, for prompt token `13041`. Both sides wrote 45 records of 4096 floats. Layers 0, 1, and 2 matched at cosine `0.99999621`, `0.99999646`, and `0.99999603`, with max abs `0.000111734`, `0.000113288`, and `0.000307988`. Layer 3, the first MLA and MoE layer, was the first max abs above `0.001`: cosine `0.99999487`, max abs `0.00429773`. The gap widened later. Layer 44 cosine was `0.99737925`, max abs `8.99907`. The shipped argmax stayed `154822` logit `20.9312` second `315`. The official learned-norm argmax on the same walk stayed `154822` logit `22.375` second `315`. The layer-3 gap does not change the token, so the shipped forward was not edited. Host MemAvailable was 120838700 KiB before the shipped run and 122781036 KiB after the files were removed.
